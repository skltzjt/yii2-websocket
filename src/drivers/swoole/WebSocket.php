...
namespace wtwt\websocket\swoole;

use wtwt\websocket\cli\WebSocket as CliWebSocket;
...
class WebSocket extends CliWebSocket
{
     ...
     */
    public function send($data, $type = 'text', int $flags = SWOOLE_WEBSOCKET_FLAG_FIN)
    {
        $this->connect();
        ...
        // 将WebSocket消息打包并发送
        return $this->_socket->send(\swoole_websocket_server::pack(json_encode($data), $_type, $flags));
    }
